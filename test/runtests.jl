...
@test all(w -> bitstring(VectorizationBase.mask(Val(16), w)) == reduce(*, (16 - i < w ? "1" : "0" for i in 1:16)), 1:16)
        ...
        @test all(Mask{4}(0xfc) + Mask{4}(0xcf) == SVec(0x01,0x01,0x02,0x02))

        @test !Mask{8}(0xfc) == Mask{8}(0x03)
        @test (!Mask{8}(0xfc) & Mask{8}(0xfc)) == Mask{8}(0x00)
        @test (!Mask{8}(0xfc) | Mask{8}(0xfc)) == Mask{8}(0xff)
end
