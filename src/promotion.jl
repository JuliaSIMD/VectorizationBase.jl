@inline Base.promote(v1::AbstractSIMD{W,Float16}, v2::AbstractSIMD{W,Float16}) where {W} =
  (convert(Float32, v1), convert(Float32, v2))
@inline Base.promote(
...
ff_promote_rule(
  ...
  ::Type{T2},
  ::Val{W},
) where {T1<:FloatingTypes,T2<:FloatingTypes,W} = _ff_promote_rule(T1, T2, Val{W}())
...
Base.promote_rule(
  ...
  ::Type{T2},
) where {N,W,T1,V,T2<:NativeTypes} = _assemble_vec_unroll(Val{N}(), promote_type(V, T2))
Base.promote_rule(
...
Base.promote_rule(::Type{V}, ::Type{T}) where {W,TV,V<:AbstractSIMD{W,TV},T<:Rational} =
