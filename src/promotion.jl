...
symetric_promote_rule(::Type{T1}, ::Type{T2}) where {T1,T2} = pick_promote(promote_rule(T1,T2), promote_rule(T2,T1))

ff_promote_rule(::Type{T1}, ::Type{T2}) where {T1,T2} = symetric_promote_rule(T1,T2)
...
Base.promote_rule(::Type{<:AbstractSIMDVector{W,T1}}, ::Type{<:AbstractSIMDVector{W,T2}}) where {W,T1,T2} = Vec{W,symetric_promote_rule(T1,T2)}


Base.promote_rule(::Type{VecUnroll{N,W,T1}}, ::Type{T2}) where {N,W,T1,T2<:NativeTypes} = VecUnroll{N,W,ff_promote_rule(T1,T2)}
Base.promote_rule(::Type{VecUnroll{N,W,T1}}, ::Type{Vec{W,T2}}) where {N,W,T1,T2} = VecUnroll{N,W,symetric_promote_rule(T1,T2)}
Base.promote_rule(::Type{VecUnroll{N,W,T1}}, ::Type{VecUnroll{N,W,T2}}) where {N,W,T1,T2} = VecUnroll{N,W,symetric_promote_rule(T1,T2)}
